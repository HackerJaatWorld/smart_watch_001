...
function setAlarm() {
    ...
    var chooseTune = document.getElementById("tunes");

    if (setTime) {
        tune.src = 'tune/' + chooseTune.value;
        // Store the alarm time in localStorage
        localStorage.setItem('alarmTime', setTime);
...
if (localStorage.getItem('alaramTime') == '') {
    ...
    setInterval(function () {
        ...
        // Retrieve the set alarm time from localStorage
        ...
        if (alarmHour === currentHour && alarmMinute === currentMinute) {
            ...
            if (confirmation) {
                ...
                localStorage.setItem('alarmTime', newAlarmHour + ':' + (newAlarmMinute < 10 ? '0' : '') + newAlarmMinute);
            }
        }
...
function modeBtn(){
    ...
    document.getElementById("set_time").classList.toggle('dark');
    document.getElementById("tunes").classList.toggle('dark');
